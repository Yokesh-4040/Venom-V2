...
function _DownloadWebGLTexture(img, size, name, ext) {
 console.log(size);
 var binary = "";
 for (var i = 0; i < size; i++) binary += String.fromCharCode(GROWABLE_HEAP_U8()[img + i]);
...
function _JS_Eval_OpenURL(ptr) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(13, 1, ptr);
 var str = UTF8ToString(ptr);
...
function _JS_FileSystem_Sync() {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(14, 0);
...
function _JS_MobileKeyboard_GetKeyboardStatus() {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(28, 1);
...
function _JS_MobileKeyboard_GetText(buffer, bufferSize) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(29, 1, buffer, bufferSize);
...
function _JS_MobileKeyboard_GetTextSelection(outStart, outLength) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(30, 1, outStart, outLength);
 outStart = outStart >>= 2;
...
function _JS_MobileKeyboard_Hide(delay) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(31, 1, delay);
...
function _JS_MobileKeyboard_SetCharacterLimit(limit) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(32, 1, limit);
 if (!mobile_input) return;
 mobile_input.input.maxLength = limit;
}

function _JS_MobileKeyboard_SetText(text) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(33, 1, text);
 if (!mobile_input) return;
...
function _JS_MobileKeyboard_SetTextSelection(start, length) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(34, 1, start, length);
 if (!mobile_input) return;
...
function _JS_MobileKeyboard_Show(text, keyboardType, autocorrection, multiline, secure, alert, placeholder, characterLimit) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(35, 1, text, keyboardType, autocorrection, multiline, secure, alert, placeholder, characterLimit);
...
function _JS_OrientationSensor_Start(callback, frequency) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(40, 1, callback, frequency);
...
function _JS_ScreenOrientation_Lock(orientationLockType) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(44, 1, orientationLockType);
...
function _JS_SetMainLoop(func, fps, simulateInfiniteLoop) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(45, 1, func, fps, simulateInfiniteLoop);
 try {
...
function _JS_Sound_Create_Channel(callback, userData) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(46, 1, callback, userData);
...
function _JS_Sound_GetAudioBufferSampleRate(soundInstance) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(47, 1, soundInstance);
...
function _JS_Sound_GetLoadState(bufferInstance) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(49, 1, bufferInstance);
...
function _JS_Sound_GetMetaData(bufferInstance, metaData) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(50, 1, bufferInstance, metaData);
 metaData = metaData >> 2;
...
function _JS_Sound_IsStopped(channelInstance) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(52, 1, channelInstance);
...
function _JS_Sound_Load(ptr, length, decompress, fmodSoundType) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(53, 1, ptr, length, decompress, fmodSoundType);
 if (WEBAudio.audioWebEnabled == 0) return 0;
 ptr = ptr;
...
function _JS_Sound_Load_PCM(channels, length, sampleRate, ptr) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(54, 1, channels, length, sampleRate, ptr);
 if (WEBAudio.audioWebEnabled == 0) return 0;
...
function _JS_Sound_Play(bufferInstance, channelInstance, offset, delay) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(55, 1, bufferInstance, channelInstance, offset, delay);
 if (WEBAudio.audioWebEnabled == 0) return;
...
function _JS_Sound_ReleaseInstance(instance) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(56, 0, instance);
...
function _JS_Sound_Set3D(channelInstance, spatialBlendLevel) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(58, 1, channelInstance, spatialBlendLevel);
 var channel = WEBAudio.audioInstances[channelInstance];
 channel.set3D(spatialBlendLevel);
}

function _JS_Sound_SetListenerOrientation(x, y, z, xUp, yUp, zUp) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(59, 1, x, y, z, xUp, yUp, zUp);
 if (WEBAudio.audioWebEnabled == 0) return;
 x = -x;
 y = -y;
 z = -z;
...
function _JS_Sound_SetListenerPosition(x, y, z) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(60, 1, x, y, z);
 if (WEBAudio.audioWebEnabled == 0) return;
 var l = WEBAudio.audioContext.listener;
 if (l.positionX) {
...
function _JS_Sound_SetLoop(channelInstance, loop) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(61, 1, channelInstance, loop);
 if (WEBAudio.audioWebEnabled == 0) return;
 var channel = WEBAudio.audioInstances[channelInstance];
...
function _JS_Sound_SetLoopPoints(channelInstance, loopStart, loopEnd) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(62, 1, channelInstance, loopStart, loopEnd);
 if (WEBAudio.audioWebEnabled == 0) return;
 var channel = WEBAudio.audioInstances[channelInstance];
...
function _JS_Sound_SetPaused(channelInstance, paused) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(63, 1, channelInstance, paused);
 if (WEBAudio.audioWebEnabled == 0) return;
 var channel = WEBAudio.audioInstances[channelInstance];
...
function _JS_Sound_SetPosition(channelInstance, x, y, z) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(65, 1, channelInstance, x, y, z);
 if (WEBAudio.audioWebEnabled == 0) return;
 var channel = WEBAudio.audioInstances[channelInstance];
...
function _JS_Sound_SetVolume(channelInstance, v) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(66, 1, channelInstance, v);
 if (WEBAudio.audioWebEnabled == 0) return;
 try {
  var channel = WEBAudio.audioInstances[channelInstance];
  channel.setVolume(v);
...
function _JS_Sound_Stop(channelInstance, delay) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(67, 1, channelInstance, delay);
 if (WEBAudio.audioWebEnabled == 0) return;
 var channel = WEBAudio.audioInstances[channelInstance];
 channel.stop(delay);
}
...
function _JS_SystemInfo_GetCanvasClientSize(domElementSelector, outWidth, outHeight) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(69, 1, domElementSelector, outWidth, outHeight);
 ...
 if (Module.WebXR && Module.WebXR.isInXR) {
  GROWABLE_HEAP_F64()[outWidth >> 3] = canvas ? canvas.width : 0;
  GROWABLE_HEAP_F64()[outHeight >> 3] = canvas ? canvas.height : 0;
  return;
 }
 GROWABLE_HEAP_F64()[outWidth >> 3] = canvas ? canvas.clientWidth : 0;
 GROWABLE_HEAP_F64()[outHeight >> 3] = canvas ? canvas.clientHeight : 0;
}
...
function _JS_SystemInfo_GetOS(buffer, bufferSize) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(73, 1, buffer, bufferSize);
...
function _JS_SystemInfo_GetScreenSize(outWidth, outHeight) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(75, 1, outWidth, outHeight);
 outWidth = outWidth >>= 3;
...
function _JS_WebRequest_Send(requestId, ptr, length, arg, onresponse, onprogress) {
 ...
 function HandleSuccess(response, body) {
  ...
  if (requestOptions.init.enableStreamingDownload) {
   ...
   GROWABLE_HEAP_U8().set(body, buffer >>= undefined);
 ...
 function HandleProgress(e) {
  ...
  if (e.chunk) {
   ...
   GROWABLE_HEAP_U8().set(e.chunk, buffer >>= undefined);
...
function ___syscall_chmod(path, mode) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(124, 1, path, mode);
 try {
...
function ___syscall_faccessat(dirfd, path, amode, flags) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(126, 1, dirfd, path, amode, flags);
 try {
  path = SYSCALLS.getStr(path);
  assert(flags === 0);
...
function ___syscall_fcntl64(fd, cmd, varargs) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(127, 1, fd, cmd, varargs);
...
function ___syscall_fstat64(fd, buf) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(128, 1, fd, buf);
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
...
function ___syscall_ftruncate64(fd, length) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(129, 1, fd, length);
 try {
...
function ___syscall_getcwd(buf, size) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(130, 1, buf, size);
 try {
  if (size === 0) return -28;
  var cwd = FS.cwd();
...
function ___syscall_getdents64(fd, dirp, count) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(131, 1, fd, dirp, count);
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
  if (!stream.getdents) {
...
function ___syscall_getsockopt(fd, level, optname, optval, optlen, d1) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(132, 1, fd, level, optname, optval, optlen, d1);
 try {
  var sock = getSocketFromFD(fd);
  if (level === 1) {
   if (optname === 4) {
...
function ___syscall_ioctl(fd, op, varargs) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(133, 1, fd, op, varargs);
 SYSCALLS.varargs = varargs;
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
  switch (op) {
...
function ___syscall_lstat64(path, buf) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(134, 1, path, buf);
 try {
  path = SYSCALLS.getStr(path);
...
function ___syscall_mkdirat(dirfd, path, mode) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(135, 1, dirfd, path, mode);
 try {
  path = SYSCALLS.getStr(path);
  path = SYSCALLS.calculateAt(dirfd, path);
  path = PATH.normalize(path);
...
function ___syscall_newfstatat(dirfd, path, buf, flags) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(136, 1, dirfd, path, buf, flags);
 try {
  path = SYSCALLS.getStr(path);
...
function ___syscall_openat(dirfd, path, flags, varargs) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(137, 1, dirfd, path, flags, varargs);
 SYSCALLS.varargs = varargs;
 try {
  path = SYSCALLS.getStr(path);
  path = SYSCALLS.calculateAt(dirfd, path);
...
function ___syscall_readlinkat(dirfd, path, buf, bufsize) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(138, 1, dirfd, path, buf, bufsize);
 try {
  path = SYSCALLS.getStr(path);
  path = SYSCALLS.calculateAt(dirfd, path);
...
function ___syscall_recvfrom(fd, buf, len, flags, addr, addrlen) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(139, 1, fd, buf, len, flags, addr, addrlen);
 try {
  var sock = getSocketFromFD(fd);
...
function ___syscall_renameat(olddirfd, oldpath, newdirfd, newpath) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(140, 1, olddirfd, oldpath, newdirfd, newpath);
 try {
...
function ___syscall_rmdir(path) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(141, 1, path);
 try {
  path = SYSCALLS.getStr(path);
  FS.rmdir(path);
  return 0;
...
function ___syscall_sendto(fd, message, length, flags, addr, addr_len) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(142, 1, fd, message, length, flags, addr, addr_len);
 try {
  var sock = getSocketFromFD(fd);
...
function ___syscall_socket(domain, type, protocol) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(143, 1, domain, type, protocol);
 try {
...
function ___syscall_stat64(path, buf) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(144, 1, path, buf);
 try {
  path = SYSCALLS.getStr(path);
...
function ___syscall_statfs64(path, size, buf) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(145, 1, path, size, buf);
 try {
  path = SYSCALLS.getStr(path);
  assert(size === 64);
...
function ___syscall_symlink(target, linkpath) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(146, 1, target, linkpath);
 try {
...
function ___syscall_truncate64(path, length) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(147, 1, path, length);
 try {
  length = bigintToI53Checked(length);
  if (isNaN(length)) return -61;
  path = SYSCALLS.getStr(path);
...
function ___syscall_unlinkat(dirfd, path, flags) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(148, 1, dirfd, path, flags);
 try {
  path = SYSCALLS.getStr(path);
  path = SYSCALLS.calculateAt(dirfd, path);
  if (flags === 0) {
...
function ___syscall_utimensat(dirfd, path, times, flags) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(149, 1, dirfd, path, times, flags);
 try {
  path = SYSCALLS.getStr(path);
  assert(flags === 0);
...
function __mmap_js(len, prot, flags, fd, off, allocated, addr) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(150, 1, len, prot, flags, fd, off, allocated, addr);
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
...
function __munmap_js(addr, len, prot, flags, fd, offset) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(151, 1, addr, len, prot, flags, fd, offset);
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
  if (prot & 2) {
...
function getCanvasSizeMainThread(target, width, height) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(153, 1, target, width, height);
...
function setCanvasElementSizeMainThread(target, width, height) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(154, 1, target, width, height);
...
function _emscripten_get_fullscreen_status(fullscreenStatus) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(156, 1, fullscreenStatus);
 if (!JSEvents.fullscreenEnabled()) return -1;
...
function _emscripten_get_gamepad_status(index, gamepadState) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(157, 1, index, gamepadState);
...
function _emscripten_request_fullscreen(target, deferUntilInEventHandler) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(159, 1, target, deferUntilInEventHandler);
 var strategy = {
  scaleMode: 0,
...
function _emscripten_request_pointerlock(target, deferUntilInEventHandler) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(160, 1, target, deferUntilInEventHandler);
 target = findEventTarget(target);
 if (!target) return -4;
 if (!target.requestPointerLock) {
  return -1;
...
function _emscripten_set_blur_callback_on_thread(target, userData, useCapture, callbackfunc, targetThread) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(162, 1, target, userData, useCapture, callbackfunc, targetThread);
 return registerFocusEventCallback(target, userData, useCapture, callbackfunc, 12, "blur", targetThread);
}

function _emscripten_set_focus_callback_on_thread(target, userData, useCapture, callbackfunc, targetThread) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(163, 1, target, userData, useCapture, callbackfunc, targetThread);
...
function _emscripten_set_fullscreenchange_callback_on_thread(target, userData, useCapture, callbackfunc, targetThread) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(164, 1, target, userData, useCapture, callbackfunc, targetThread);
 if (!JSEvents.fullscreenEnabled()) return -1;
 target = findEventTarget(target);
 if (!target) return -4;
...
function _emscripten_set_gamepadconnected_callback_on_thread(userData, useCapture, callbackfunc, targetThread) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(165, 1, userData, useCapture, callbackfunc, targetThread);
 if (_emscripten_sample_gamepad_data()) return -1;
 return registerGamepadEventCallback(2, userData, useCapture, callbackfunc, 26, "gamepadconnected", targetThread);
}

function _emscripten_set_gamepaddisconnected_callback_on_thread(userData, useCapture, callbackfunc, targetThread) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(166, 1, userData, useCapture, callbackfunc, targetThread);
 if (_emscripten_sample_gamepad_data()) return -1;
...
function _environ_get(__environ, environ_buf) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(179, 1, __environ, environ_buf);
 var bufSize = 0;
...
function _environ_sizes_get(penviron_count, penviron_buf_size) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(180, 1, penviron_count, penviron_buf_size);
...
function _fd_close(fd) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(181, 1, fd);
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
  FS.close(stream);
  return 0;
...
function _fd_fdstat_get(fd, pbuf) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(182, 1, fd, pbuf);
 try {
  var rightsBase = 0;
...
function _fd_read(fd, iov, iovcnt, pnum) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(183, 1, fd, iov, iovcnt, pnum);
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
...
function _fd_seek(fd, offset, whence, newOffset) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(184, 1, fd, offset, whence, newOffset);
 try {
...
function _fd_write(fd, iov, iovcnt, pnum) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(185, 1, fd, iov, iovcnt, pnum);
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
...
function _gethostbyaddr(addr, addrlen, type) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(186, 1, addr, addrlen, type);
 if (type !== 2) {
  setErrNo(5);
  return null;
 }
...
function _gethostbyname(name) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(187, 1, name);
...
function _wgpu_adapter_request_device_async(adapter, descriptor, deviceCallback, userData) {
 ...
 }, "GPUAdapter.requestDevice() with desc")).then(_wgpuMuteJsExceptions(cb)).catch(e => {
...
function _wgpu_command_encoder_begin_render_pass(commandEncoder, descriptor) {
 ...
 assert(descriptor % 4 == 0, "assert(descriptor % 4 == 0) failed!");
 descriptor >>= 2;
 let colorAttachments = [], numColorAttachments = GROWABLE_HEAP_I32()[descriptor++], colorAttachmentsIdx = GROWABLE_HEAP_U32()[descriptor++] >> 2, colorAttachmentsIdxDbl = colorAttachmentsIdx + 6 >> 1, depthStencilView = wgpu[GROWABLE_HEAP_U32()[descriptor]];
 ...
 while (numColorAttachments--) {
  colorAttachments.push(GROWABLE_HEAP_U32()[colorAttachmentsIdx] ? {
   ...
   "depthSlice": GROWABLE_HEAP_I32()[colorAttachmentsIdx + 1] < 0 ? undefined : GROWABLE_HEAP_I32()[colorAttachmentsIdx + 1],
...
function _wgpu_command_encoder_begin_render_pass_1color_0depth(commandEncoder, descriptor) {
 ...
 let colorAttachmentsIdx = GROWABLE_HEAP_U32()[descriptor + 1] >> 2, colorAttachmentsIdxDbl = colorAttachmentsIdx + 6 >> 1;
 assert(colorAttachmentsIdx % 2 == 0, "assert(colorAttachmentsIdx % 2 == 0) failed!");
 return wgpuStore(debugDir(wgpu[commandEncoder]["beginRenderPass"](debugDir({
  "colorAttachments": [ {
   "view": wgpu[GROWABLE_HEAP_U32()[colorAttachmentsIdx]],
...
function _wgpu_device_create_compute_pipeline(device, computeModule, entryPoint, layout, constants, numConstants) {
 ...
 assert(!entryPoint || UTF8ToString(entryPoint).length > 0, "assert(!entryPoint || UTF8ToString(entryPoint).length > 0) failed!");
 device = wgpu[device];
...
function wgpuReadRenderPipelineDescriptor(descriptor) {
 ...
 desc = {
  "vertex": {
   ...
   "entryPoint": UTF8ToString(GROWABLE_HEAP_U32()[vertexIdx + 1]) || void 0,
 ...
 if (fragmentModule) desc["fragment"] = {
  ...
  "entryPoint": UTF8ToString(GROWABLE_HEAP_U32()[fragmentIdx + 1]) || void 0,
  "targets": targets,
...
function wgpuReadShaderModuleCompilationHints(index) {
 ...
 while (numHints--) {
  ...
  hints.push({
   "entryPoint": UTF8ToString(GROWABLE_HEAP_U32()[hintsIndex]),
...
function _wgpu_queue_submit_multiple_and_destroy(queue, commandBuffers, numCommandBuffers) {
 ...
 while (commandBuffers < end) _wgpu_object_destroy(GROWABLE_HEAP_U32()[commandBuffers++]);
}
